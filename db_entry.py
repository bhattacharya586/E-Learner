...
def testsol(solutions_list,questions_list,batch,email,enrollment,subject,testname):
    ...
    ref = db.reference('/Tests/'+'DBMS'+'/Indexing and Hashing')
    m = ref.get()
    if m != None:
        solutions_real = []
        data = dict(m)
...
if m != None:
    ...
    for i,j in data.items():
        ...
        for i in range(0,len(questions)):
            sol = random.choice([optionA[i],optionB[i],optionC[i],optionD[i]])
            solutions_list.append(sol.strip())
            questions_list.append(questions[i].strip())
        print("MAIN => ", questions_list)
        print("MAIN => ", solutions_list)
        testsol(solutions_list,questions_list,j["class"],j["email"],j["university_roll_no"],"DBMS","Indexing and Hashing")
